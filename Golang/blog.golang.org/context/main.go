...
func handleSearch(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error()+"<- line 50", http.StatusBadRequest)
		return
	}
	...
	start := time.Now()
	results, err := Search(ctx, query) //There's a problem here apparently
	elapsed := time.Since(start)
	if err != nil {
		http.Error(w, err.Error()+" <- line 60", http.StatusInternalServerError)
		return
	}
