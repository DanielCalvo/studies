...
helm  --kube-context hp upgrade --install grafana grafana/grafana \
  --version 10.5.15 \
...
curl -fsS -u admin:admin http://192.168.1.231/api/datasources/name/Prometheus | jq '{name, uid, url}'
curl -fsS -u admin:admin 'http://192.168.1.231/api/search?query=' | jq '[.[] | {title, uid, url}]'
```
