...
```json
{"insecure-registries":["192.168.1.225:5000"]}
```
...
docker tag busybox:latest 192.168.1.225:5000/busybox:latest
docker push 192.168.1.225:5000/busybox:latest
curl http://192.168.1.225:5000/v2/busybox/tags/list
```
...
```yaml
image: 192.168.1.225:5000/busybox:latest
```
...
DIGEST=$(curl -sI \
  ...
  http://192.168.1.225:5000/v2/busybox/manifests/latest \
  | awk -F': ' '/Docker-Content-Digest/ {print $2}' \
  | tr -d '\r')

curl -X DELETE "http://192.168.1.225:5000/v2/busybox/manifests/${DIGEST}"
curl http://192.168.1.225:5000/v2/busybox/tags/list
```
