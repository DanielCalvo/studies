...
- `service.yaml`: MetalLB LoadBalancer pinned to `192.168.1.225`.
...
- The local Docker daemon must list `192.168.1.225:5000` in `insecure-registries`.
- Each k3s node must have `/etc/rancher/k3s/registries.yaml` pointing this registry to `http://192.168.1.225:5000`, followed by restarting `k3s` or `k3s-agent`.
...
- A previous successful test pod logged:
  - `pulled 192.168.1.225:5000/busybox:latest`
  - `aarch64`
